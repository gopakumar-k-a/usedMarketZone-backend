...
class Scripts {
    ...
    addRepeatableJobArgs(customKey, nextMillis, opts, legacyCustomKey, skipCheckExists) {
    ...
    async addRepeatableJob(customKey, nextMillis, opts, legacyCustomKey, skipCheckExists) {
        const client = await this.queue.client;
        const args = this.addRepeatableJobArgs(customKey, nextMillis, opts, legacyCustomKey, skipCheckExists);
        return client.addRepeatableJob(args);
    }
    ...
    async addLog(jobId, logRow, keepLogs) {
        ...
        const result = await client.addLog(keys.concat([jobId, logRow, keepLogs ? keepLogs : '']));
        if (result < 0) {
            throw this.finishedErrors({
                code: result,
                jobId,
    ...
    async isMaxed() {
        ...
        return !!(await client.isMaxed(args));
    }
    ...
    async getRateLimitTtl(maxJobs) {
        ...
        return client.getRateLimitTtl(args);
    }
    /**
    ...
    async moveStalledJobsToWait() {
        ...
        return client.moveStalledJobsToWait(args);
    }
    /**
